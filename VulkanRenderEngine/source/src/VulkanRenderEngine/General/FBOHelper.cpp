...
static void BindFbo(VKWrapper::VKFrameBuffer& fbo, Texture2D& colorBuffer)
{
...
static void BindFbo(VKWrapper::VKFrameBuffer& fbo, Texture2D& colorBuffer, Texture2D& depthBuffer)
{
	//glBindFramebuffer(GL_FRAMEBUFFER, fbo);
...
static void BindFbo(VKWrapper::VKFrameBuffer& fbo, Texture2D& colorBuffer, Texture2D& brightColorBuffer, Texture2D& depthBuffer, uint32_t width, uint32_t height)
{
	//VKWrapper::FrameBufferConfig config;
	//config.device = _vulkanDevice.get();
	//config.renderPass = _renderPass.get();
