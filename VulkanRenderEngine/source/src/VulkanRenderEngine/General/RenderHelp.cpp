...
void SetupDirLightData(std::shared_ptr<VKWrapper::VKCommandBuffer>& cmd, Pipeline& pipeline, const std::vector<std::shared_ptr<DirLightInfo>>& dirLights, const std::shared_ptr<AtlasMap>& atlasShadowMap)
{
	...
	auto dirLightMeta_ssbo = pipeline.FindStorageBlock(GeneralBindingPoint::Light_DirLightMetaData);
	if (!dirLightMeta_ssbo)
	{
		pipeline.SetStorageBlock(meta_ssbo, GeneralBindingPoint::Light_DirLightMetaData);
		dirLightMeta_ssbo = meta_ssbo;
	}

	auto dirLightCascade_ssbo = pipeline.FindStorageBlock(GeneralBindingPoint::Light_DirLightCascadeData);
	if (!dirLightCascade_ssbo)
	{
		pipeline.SetStorageBlock(cascade_ssbo, GeneralBindingPoint::Light_DirLightCascadeData);
...
void SetupPointLightData(std::shared_ptr<VKWrapper::VKCommandBuffer>& cmd, Pipeline& pipeline, const std::vector<std::shared_ptr<PointLightInfo>>& pointLights, const std::shared_ptr<AtlasMap>& atlasShadowMap)
{
	...
	auto pointLightMeta_ssbo = pipeline.FindStorageBlock(GeneralBindingPoint::Light_PointLightMetaData);
	if (!pointLightMeta_ssbo)
	{
		pipeline.SetStorageBlock(meta_ssbo, GeneralBindingPoint::Light_PointLightMetaData);
...
void SetupSpotLightData(std::shared_ptr<VKWrapper::VKCommandBuffer>& cmd, Pipeline& pipeline, const std::vector<std::shared_ptr<SpotLightInfo>>& spotLights, const std::shared_ptr<AtlasMap>& atlasShadowMap)
{
	...
	auto spotLightMeta_ssbo = pipeline.FindStorageBlock(GeneralBindingPoint::Light_SpotLightMetaData);
	if (!spotLightMeta_ssbo)
	{
		pipeline.SetStorageBlock(meta_ssbo, GeneralBindingPoint::Light_SpotLightMetaData);
		spotLightMeta_ssbo = meta_ssbo;
	}
