...
{
	_SSBO_WorldBVHNode = std::make_shared<StorageBlock>();
	_SSBO_MeshmatData = std::make_shared<StorageBlock>();
}

std::shared_ptr<StorageBlock> RayTraceGeneralBuffer::GetTraiangles()
{
	return _traiangleBufferManager.GetBuffer()->GetBlock();
}

std::shared_ptr<StorageBlock> RayTraceGeneralBuffer::GetTraiangleExt()
{
	return _traiangleExtBufferManager.GetBuffer()->GetBlock();
}

std::shared_ptr<StorageBlock> RayTraceGeneralBuffer::GetMeshBVHNode()
{
	return _meshBVHNodeBufferManager.GetBuffer()->GetBlock();
}

std::shared_ptr<StorageBlock> RayTraceGeneralBuffer::GetMeshmatData()
{
	return _SSBO_MeshmatData;
}

std::shared_ptr<StorageBlock> RayTraceGeneralBuffer::GetWorldBVHNode()
{
...
bool RayTraceGeneralPass::SetupGeneralBuffer(RenderState& state)
{
	...
	auto worldBVHData = BVHBuilder::Build(meshAABBData, GlobalConfig::RayTrace_World_BVH_Leaf_MeshCount, GlobalConfig::RayTrace_Max_Recursive_Depth);
