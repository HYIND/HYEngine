...
bool EffectPass::ShouldExecute(RenderGraph::FrameDataRegistry& registry, RenderState& state)
{
	return !state.objects.sceneRenderData.effectItems.empty();
}

void EffectPass::Execute(RenderGraph::FrameDataRegistry& registry, const RenderGraph::PassContext& ctx, RenderState& state)
{
