...
bool RayTracePass::SetupMeshBuffer(Shader& shader, RenderState& state)
{
	...
	for (auto& item : state.objects.sceneRenderData.opaqueMesh)
	{
		...
		//SegmentData meghBVHIndicesSegData;
	...
	{
		traiangleBuffer->WriteData(_traiangleBufferManager.GetBuffer()->GetData(), _traiangleBufferManager.GetUseSpace(), 0);
		traiangleExtBuffer->WriteData(_traiangleExtBufferManager.GetBuffer()->GetData(), _traiangleExtBufferManager.GetUseSpace(), 0);
		meshBVHNodeBuffer->WriteData(_meshBVHNodeBufferManager.GetBuffer()->GetData(), _meshBVHNodeBufferManager.GetUseSpace(), 0);
