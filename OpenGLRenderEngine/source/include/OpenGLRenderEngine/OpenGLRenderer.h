...
public:
	...
	void Init(uint32_t width, uint32_t height, SharedTexture* sharedTexture = nullptr);
	void Draw(RenderState& state);

	GLuint GetColorBuffer() const;
	GLuint GetColorFBO() const;
	int GetWidth() const;
	int GetHeight() const;
...
private:
	...
	struct {
		...
		GLuint finalFbo = 0, finalColorBuffer = 0;
