...
struct CollisionShape
{
	...
	void AddBoxShape(const glm::vec3& size, glm::vec3 position = glm::vec3(0.0f), glm::quat rotation = glm::identity<glm::quat>())
	{
	...
	void AddSphereShape(float radius, glm::vec3 position = glm::vec3(0.0f), glm::quat rotation = glm::identity<glm::quat>())
	{
	...
	void AddCapsuleShape(float radius, float height, glm::vec3 position = glm::vec3(0.0f), glm::quat rotation = glm::identity<glm::quat>())
	{
		radius = std::max(0.f, radius);
	...
	void AddCylinderShape(float radius, float height, glm::vec3 position = glm::vec3(0.0f), glm::quat rotation = glm::identity<glm::quat>())
	{
		radius = std::max(0.f, radius);
		height = std::max(0.f, height);
		auto shape = std::make_shared<ChildShape>();
...
struct Physics :public IComponent
{
	...
	enum class BodyType {
		Static,    // 质量 = 0，不动
		Dynamic,   // 质量 > 0，受物理影响
		Kinematic  // 质量 = 0，setKinematic(true)，可手动控制
	};
