...
Mesh::Mesh(std::vector<Vertex> vertices, std::vector<unsigned int> indices)
	:VAO(0), VBO(0), EBO(0),
	needUpdateVerticesIndices(true), needUpdateBVH(true), needUpdateAABB(true),
	_viVersion{ 0 }, _bvhversion{ 0 }, _aabbversion{ 0 }
{
	...
	setupMesh();
	needUpdateVerticesIndices = false;
}
...
void Mesh::setupMesh()
{
	needUpdateAABB = true;
	needUpdateBVH = true;
	needUpdateVerticesIndices = true;
	_viVersion++;
	_bvhversion++;
	_aabbversion++;
...
void Mesh::MakeScale(const glm::vec3& scale, bool needSetupMesh)
{
	...
	MakeTransform(mat, needSetupMesh);
}

void Mesh::MakeTranslate(const glm::vec3& trans, bool needSetupMesh)
{
...
void Mesh::MakeRotate(float angle, const glm::vec3& axis, bool needSetupMesh)
{
	if (angle == 0.0f)
		return;
...
void Mesh::MakeTransform(const glm::mat4& mat, bool needSetupMesh)
{
