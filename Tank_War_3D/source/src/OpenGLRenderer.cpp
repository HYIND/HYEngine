...
void HandleKeyboardShortcuts(RenderState& state)
{
	...
	static bool s_ssrOn = state.flags.ssrOn;
	static bool s_useGbuffer = state.rayTraceParams.useGbuffer;
	static bool s_ssgiOn = state.flags.ssgiOn;
	...
	state.flags.rayTraceOn = s_rayTraceEnable;
	state.flags.ssrOn = s_ssrOn;
	state.rayTraceParams.useGbuffer = s_useGbuffer;
	state.flags.ssgiOn = s_ssgiOn;
}
...
void OpenGLRenderer::Init(int width, int height, SharedTexture* sharedTexture)
{
	scr_width = width;
	scr_height = height;
...
void OpenGLRenderer::Draw(RenderState& state)
{
	...
	if (state.flags.bloomOn) _globalBloomPass->Draw(_renderTarget.combinBrightColorBuffer->GetID());
	_globalPostProcessPass->Draw(
		_renderTarget.finalFbo, _renderTarget.combinColorBuffer->GetID(), _globalBloomPass->GetBloomBlurMap(),
		state.flags.bloomOn, state.flags.gammaOn, needFlipFinalFboY,
		pow(2.0f, state.common.EV100), state.common.gamma
	);
...
void OpenGLRenderer::InitSceneRenderGraph()
{
	...
	ssgiPass->SetRenderPass(std::make_unique<SSGIPass>("shader/ssr/SSGI.comp", "shader/ssr/BilateralFilterBlur.comp", "shader/ssr/TemporalAccumulate.comp"))
		.Input(gPosition, gNormal, gAlbedoOpacity, gMetallicRoughnessMap, gMotionVectorMap, ssaoOutPut)
	...
	autoExposurePass->SetEnable(false);
...
void OpenGLRenderer::InitFirstPersonRenderGraph()
{
	...
	auto CreateTexture = [&](int width, int height, GLenum format, GLenum filterMode, GLenum wrapMode, const std::string& name)
		->OpenGLRenderGraph::RenderGraphResource {
		return _firstPersonRenderGraph->CreateTexture(OpenGLRenderGraph::TextureDesc{ width ,height,format,filterMode,wrapMode }, name);
		};
...
{
	_record.prevEV100 = state.common.EV100;
