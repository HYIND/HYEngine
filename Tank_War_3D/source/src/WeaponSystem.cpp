...
void WeaponSystem::update(float deltaTime)
{
	...
	for (auto& entity : entities)
	{
		...
		if (auto* ani = weapon.tryGetComponent<SkeletonAnimatorGroup>())
		{
			if (!ani->HasAnimatorPlaying())
				ani->PlayAnimator("Idle", 0);
		}
	}
}
