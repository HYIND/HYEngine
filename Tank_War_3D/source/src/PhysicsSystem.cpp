...
btCollisionShape* createShape(CollisionShape::ChildShape& childShape)
{
...
btCollisionShape* createCompoundShape(std::vector<std::shared_ptr<CollisionShape::ChildShape>>& childShapes)
{
	...
	{
		// 2.1 创建子形状
		btCollisionShape* childShape = createShape(*child);
...
void PhysicsSystem::syncWorldToPhysics(float dt_second)
{
	...
	for (auto entity : entities)
	{
		...
		{
			// 从 Movement 组件读取输入
			if (auto* move = entity.tryGetComponent<CharacterMovement>())
			{
		...
		// ========================================
		// 情况2：Kinematic 刚体（手动控制位置）
		// ========================================
		if (physics.body && physics.bodyType == Physics::BodyType::Kinematic) {
		...
		// ========================================
		// 情况3：Dynamic 刚体（由物理驱动）
		// ========================================
		if (physics.body && physics.bodyType == Physics::BodyType::Dynamic) {
			//if (auto* move = entity.tryGetComponent<Movement>()) {
			//	glm::vec3 direction = trans->rotation * glm::vec3(0, 0, -1);
			//	btVector3 velocity(
			//		direction.x * move->currentMoveSpeed,
			//		direction.y * move->currentMoveSpeed,
			//		direction.z * move->currentMoveSpeed
			//	);
			//	physics.body->setLinearVelocity(velocity);
			//	physics.body->setAngularVelocity(btVector3(
			//		0,
			//		Tool::AngleToRadian(move->currentRotationSpeed),
			//		0
			//	));
			//}
		}
...
void PhysicsSystem::createBody(Entity entity, Physics& physics, Transform& trans)
{
	...
	if (physics.isCharacter) {
		if (physics.collisionShape.children.empty())
			return;
		physics.collisionShape.shapePtr = createShape(*physics.collisionShape.children[0]);
		createCharacter(entity, physics, trans);
	}
	else {
		physics.collisionShape.shapePtr = createCompoundShape(physics.collisionShape.children);
