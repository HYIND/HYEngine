...
public:
	...
	virtual IComponent* tryGetComponentAsIComponent(const EntityID& entityId) = 0;
	virtual void duplicateComponent(Entity& newEntity, const Entity& srcEntity) = 0;
};
...
public:
	...
	T& addComponent(const Entity& entity, Args&&... args)
	{
		...
		new (&componentData[newIndex]) T(std::forward<Args>(args)...);
		try {
	...
	bool addComponent(const Entity& entity, const T& component)
	{
		EntityID entityId = entity.getId();
		assert(entityToIndex.find(entityId) == entityToIndex.end() &&
			"Entity already has this component!");
		...
		new (&componentData[newIndex]) T(component);
		try {
			T* elementPtr = &componentData[newIndex];
			if (IComponent* com = dynamic_cast<IComponent*>(elementPtr))
				com->OnAdd(entity);
	...
	IComponent* tryGetComponentAsIComponent(const EntityID& entityId) override {
		if (!hasComponent(entityId))
			return nullptr;
		T* comp = &componentData[entityToIndex[entityId]];
		if constexpr (std::is_base_of_v<IComponent, T>) {
			return static_cast<IComponent*>(comp);
		}
		return nullptr;
	}

	void duplicateComponent(Entity& newEntity, const Entity& srcEntity)
	{
		EntityID entityId = srcEntity.getId();
		if (!hasComponent(entityId))
			return;

		const T& srcComp = componentData[entityToIndex[entityId]];
		addComponent(newEntity, srcComp);
	}

	bool hasComponent(const EntityID& entityId) const {
		return !entityToIndex.empty() && entityToIndex.find(entityId) != entityToIndex.end();
	}
