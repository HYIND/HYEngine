...
private:
	...
	public:
		...
		void invoke(void* eventData) override
		{
			try
			{
				_callback(*static_cast<T*>(eventData));
			}
...
public:
	...
	void Subscribe(SystemTypeID systemId, std::function<void(Event&)> callback)
	{
		EventTypeIDToCallbackMap& map = SystemIDToEvent[systemId];
