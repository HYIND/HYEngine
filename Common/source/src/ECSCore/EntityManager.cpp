...
EntityID EntityManager::DuplicateEntity(EntityID srcEntityId)
{
	auto newEntityId = createEntity();

	if (isValid(srcEntityId))
	{
		auto& srcMask = componentMasks[srcEntityId];
		componentMasks[newEntityId] = srcMask;
	}
